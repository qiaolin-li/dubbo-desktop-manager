<template>
    <div class="log-console-container">
        <codemirror ref="cm" v-model="code" :options="cmOptions"  @input="inputChange"></codemirror>
    </div>
</template>
...
export default {
    ...
    methods: {
        ...
        appendLog(message) {
            ...
            const lastVisibleLine = cm.lineAtHeight(
                scrollInfo.top + scrollInfo.clientHeight,
                "local"
            );
            const isLastLineVisible = lastVisibleLine >= cm.lineCount() - 1; // 判断最后一行是否可见
            const lastLine = doc.lastLine();
            const lastLineLength = doc.getLine(lastLine).length;

            doc.replaceRange(message, { line: lastLine, ch: lastLineLength });

            // 如果最后一行可见，则滚动到底部
            if (isLastLineVisible) {
                const targetLine = doc.lastLine();
                const targetCh = doc.getLine(targetLine).length;
...
.toolbar {
    ...
    background-color: var(--app-theme-workbench-panel-bg);
