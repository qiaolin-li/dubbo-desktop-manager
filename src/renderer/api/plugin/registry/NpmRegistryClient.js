...
export default class NpmRegistryClient {
    ...
    buildSearchText(pluginNameKeyword, keyword) {
        return keyword ? `keywords:${pluginNameKeyword},${keyword}` : `keywords:${pluginNameKeyword}`;
    }
    ...
    getDownloads(item) {
        return item?.downloads?.all ?? item?.downloads?.monthly ?? 0;
    }
