...
```
注册的key默认会加上 `pluginLocale.${pluginName}.`前缀，防止和系统语言包冲突，我们可以通过`xx.t`方法来获取
...
// 获取当前插件的方言
this.pluginT(`pluginLocale.${pluginName}.${key}`);

// 主进程使用
app.t(`pluginLocale.${pluginName}.${key}`);
...
// 获取当前插件的方言
this.t(`pluginLocale.${pluginName}.${key}`);
